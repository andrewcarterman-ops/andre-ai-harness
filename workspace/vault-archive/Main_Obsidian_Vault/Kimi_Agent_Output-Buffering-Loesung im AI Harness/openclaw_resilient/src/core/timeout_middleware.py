"""
SCHRITT 3: Timeout Middleware
=============================
Diese Datei implementiert das 3-Tier-Timeout-Handling für Streams.
Kopiere nach: src/core/timeout_middleware.py

Warum: Ein einzelner Timeout reicht nicht. Wir brauchen:
  1. First-Token-Timeout (Verbindungsprobleme)
  2. Stall-Timeout (Stream hängt)
...
class StreamMetrics:
    ...
    def __str__(self) -> str:
        return (
            f"StreamMetrics(ttft={self.ttft:.2f}s, "
...
class StreamingTimeoutMiddleware:
    ...
    Wrappt beliebige Async Generators (OpenClaw Streams, OpenAI, etc.)
...
def create_middleware_for_model(
    ...
    if custom_timeouts:
        return StreamingTimeoutMiddleware(
            ...
            total_timeout=custom_timeouts.get(
                "total", profile.total_timeout_seconds
            )
        )
    ...
    return StreamingTimeoutMiddleware(
        ...
        total_timeout=profile.total_timeout_seconds
    )
